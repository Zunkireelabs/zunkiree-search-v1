...
export function CollapsedBar({
  ...
  return (
    <>
      {/* FAB — pill button, bottom-right (mobile always, desktop after minimize) */}
      <button
        className={`zk-fab ${minimized ? 'zk-fab--desktop' : ''} ${visible ? 'zk-fab--visible' : ''}`}
        onClick={onClick}
      ...
      {/* Desktop full bar — only on first load, hidden after minimize */}
      {!minimized && (
